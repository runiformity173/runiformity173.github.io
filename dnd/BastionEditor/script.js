...
function changeRoomType(newRoomType) {
    roomTypes[room] = newRoomType;
    if (roomTypes[room] == "Unassigned") roomTypes[room] = undefined;
...
function validateBastion() {
    for (let i = 0;i<roomTypes.length;i++) {
        if (!roomTypes[i]) continue;
        const actual = document.getElementsByClassName("room-"+i).length;
        const expected = roomSizes[premadeRooms[roomTypes[i]].space];
        if (actual > expected) {
            alert("Room " + i + " has the wrong number of tiles. It has " + actual + " when it should " + (actual>expected?"only ":"") + "have " + expected + ".");
            return;
        }
    }
    alert("All good!")
