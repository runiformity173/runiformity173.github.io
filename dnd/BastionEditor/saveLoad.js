...
function setCookie(value) {
    localStorage.setItem("bastionEditor", btoa(JSON.stringify(value)));
}
function compressCookie(amount=6) {
    localStorage.setItem("bastionEditor", customEncode(localStorage.getItem("bastionEditor"),amount));
}
function getCookie() {
    return JSON.parse(atob(customDecode(localStorage.getItem("bastionEditor")) || "e30="));
}
function load() {
    ...
    if (!data.board) {
        ...
        // document.getElementById("bastionRoomSelect").value = roomTypes[newRoom] || "Unassigned";
